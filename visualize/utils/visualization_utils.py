...
def show_image_with_boxes(img, objects, color, show3d=False):
    ...
    for obj in objects:
        ...
        box3d_pts_2d, box3d_pts_3d = kitti_data_utils.compute_box_3d(obj, cnf.P2)
...
def invert_target(targets, img_shape_2d, RGB_Map=None):
    predictions = targets
    ...
    for index, l in enumerate(predictions):
        ...
        _, corners_3d = kitti_data_utils.compute_box_3d(obj, cnf.P2)
        corners3d.append(corners_3d)
        objects_new.append(obj)

    if len(corners3d) > 0:
        corners3d = np.array(corners3d)
        img_boxes, _ = kitti_data_utils.corners3d_to_img_boxes(corners3d, cnf.P2)
    ...
    if RGB_Map is not None:
        ...
        if not noObjectLabels:
            labels[:, 1:] = transformation.camera_to_lidar_box(labels[:, 1:])#, calib.V2C, calib.R0,calib.P)  # convert rect cam to velo cord
...
def predictions_to_kitti_format(img_detections, img_shape_2d, img_size, RGB_Map=None):
    predictions = []
