...
from evaluate_new import evaluate_mAP
...
def main_worker(gpu_idx, configs):
    ...
    if configs.pretrained_path is not None:
        ...
        model.load_state_dict(torch.load(configs.pretrained_path, map_location='cuda:{}'.format(configs.gpu_idx)))
    ...
    if configs.resume_path is not None:
        ...
        model.load_state_dict(torch.load(configs.resume_path, map_location='cuda:{}'.format(configs.gpu_idx)))
        if logger is not None:
    ...
    if configs.evaluate:
        ...
        precision, recall, AP, f1, ap_class, _ = evaluate_mAP(val_dataloader, model, configs, None)
    ...
    for epoch in range(configs.start_epoch, configs.num_epochs + 1):
        ...
        # train for one epoch
        train_loss = train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer)
        if not configs.no_val:
            ...
            if tb_writer is not None:
                ...
                tb_writer.add_scalars('Train and Validation Loss', loss_dict, epoch)
...
def train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer):
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_dataloader)):
        ...
        #print('Reduced Loss is: {}'.format(reduced_loss))
        losses.update(to_python_float(reduced_loss), batch_size)
        #print('Training Loss is: {}'.format(losses))
...
def validate(val_dataloader, model, configs):
    ...
    progress = ProgressMeter(len(val_dataloader), [batch_time, data_time, losses],
