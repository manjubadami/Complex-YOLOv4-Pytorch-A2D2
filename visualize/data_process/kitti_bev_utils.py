...
def makeBVFeature(PointCloud_, Discretization, bc):
    ...
    _, indices = np.unique(PointCloud[:, 0:2].astype('int32'), axis=0, return_index=True)
    ...
    _, indices, counts = np.unique(PointCloud[:, 0:2].astype('int32'), axis=0, return_index=True, return_counts=True)
    PointCloud_top = PointCloud[indices]

    normalizedCounts = np.minimum(1.0, np.log(counts + 1) / np.log(64))

    intensityMap[np.int_(PointCloud_top[:, 0]), np.int_(PointCloud_top[:, 1])] = np.minimum(1.0,PointCloud_top[:, 3])
    ...
    #cv2.imwrite('Density.jpg',RGB_Map[2, :, :]*255)
    #cv2.imwrite('Height.jpg',RGB_Map[1, :, :]*255)
    #cv2.imwrite('Intensity.jpg',RGB_Map[0, :, :]*255)
