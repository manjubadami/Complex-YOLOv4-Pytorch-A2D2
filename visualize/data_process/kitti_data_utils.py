...
"""
def corners3d_to_img_boxes(corners3d, P):
    '''
    ...
    img_pts = np.matmul(corners3d_hom, P.T)  # (N, 8, 3) np.transpose
...
def axis_angle_to_yaw(axis, angle):
    rotation_mat = np.cos(angle) * np.eye(3) + \
...
def read_label(label_filename):
    ...
    with open (label_filename, 'r') as f:
        bboxes = json.load(f)
    lines = [] # a list for containing bounding boxes  
    for bbox in bboxes.keys():        
        class_type = bboxes[bbox]['class']
        truncation = bboxes[bbox]['truncation']
        occlusion = bboxes[bbox]['occlusion']
        alpha = bboxes[bbox]['alpha']
        x_min = bboxes[bbox]['2d_bbox'][0]
        y_min = bboxes[bbox]['2d_bbox'][1]
        x_max = bboxes[bbox]['2d_bbox'][2]
        y_max = bboxes[bbox]['2d_bbox'][3]        
        h = bboxes[bbox]['size'][2]
        w = bboxes[bbox]['size'][1]
        l = bboxes[bbox]['size'][0]
        x = bboxes[bbox]['center'][0]
        y = bboxes[bbox]['center'][1]
        z = bboxes[bbox]['center'][2]
        yaw = axis_angle_to_yaw(bboxes[bbox]['axis'], bboxes[bbox]['rot_angle'])        
        kitti_str = '%s %d %d %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f' \
                    % (class_type, int(truncation), int(occlusion), alpha, x_min, y_min, x_max, y_max, h, w, l, x, y, z, yaw)        
