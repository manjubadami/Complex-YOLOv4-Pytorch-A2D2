...
from utils.visualization_utils import show_image_with_boxes, merge_rgb_to_bev, predictions_to_kitti_format, invert_target
...
def parse_test_configs():
    ...
    if configs.save_test_output:
        configs.results_dir = os.path.join(configs.working_dir, 'results_with_val', configs.saved_fn)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for batch_idx, (img_paths, imgs_bev, targets) in enumerate(val_dataloader):
            ...
            img_rgb = cv2.undistort(img_rgb, cnf.DM, distCoeffs=cnf.D, newCameraMatrix=cnf.UDM)
            objects_pred_gt = invert_target(targets[:, 1:], img_rgb.shape, RGB_Map=None)
            img_rgb = show_image_with_boxes(img_rgb, objects_pred_gt, [0, 0, 255], False)
            ...
            #img_bev = 255 - img_bev
            #top_left = (0, 0)
            #bottom_right = (607, 607)
            #color = (0, 0, 0)  # BGR color format (here, green color)
            #thickness = 1  # Border thickness in pixels
            #cv2.rectangle(img_bev, top_left, bottom_right, color, thickness)
            # Rescale target
            targets[:, 2:6] *= configs.img_size
            # Get yaw angle
            targets[:, 6] = torch.atan2(targets[:, 6], targets[:, 7])
            
            #drawing grount truth
            for c, x, y, w, l, yaw in targets[:, 1:7].numpy():
                # Draw rotated box
                kitti_bev_utils.drawRotatedBox(img_bev, x, y, w, l, yaw, [0, 0, 255])
            ...
            for detections in img_detections:
                ...
                for x, y, w, l, im, re, *_, cls_pred in detections:
                    ...
                    # Draw rotated box
                    kitti_bev_utils.drawRotatedBox(img_bev, x, y, w, l, yaw, [0, 255, 0])                
            ...
            objects_pred = predictions_to_kitti_format(img_detections, img_rgb.shape, configs.img_size)
            img_rgb = show_image_with_boxes(img_rgb, objects_pred, [0, 255, 0], False)
