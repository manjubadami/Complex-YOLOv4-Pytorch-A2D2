...
class KittiDataset(Dataset):
    def __init__(self, dataset_dir, mode='train', lidar_transforms=None, aug_transforms=None, multiscale=False,
        ...
        self.image_idx_list = [x.split() for x in open(split_txt_path).readlines()]
        self.num_samples_counter = 0
        self.lidar_id_combination_idx = 0
        #self.ct = 0
        ...
        if self.is_test:
            ...
            if self.mode == 'train':
                ...
                self.ten_persent_list = self.sample_id_list[:int(len(self.sample_id_list)*0.2)]
                print(len(self.ten_persent_list))
            else:
                self.ten_persent_list = []
                print(len(self.ten_persent_list))
    ...
    def load_img_only(self, index):
        ...
        list_of_lidar_ids_considered = [0,2,3] # ID0:Left, ID3:Centre, ID2:Right, ID1:Rear_Right, ID4:Rear_Left
        lidarData = self.get_lidar(sample_id, list_of_lidar_ids_considered)
        ...
        img_file = self.get_image(sample_id)
    ...
    def load_img_with_targets(self, index):
        ...
        sample_id = self.sample_id_list[index]
        list_of_lidar_ids_considered = [0,3,2] # ID0:Left, ID3:Centre, ID2:Right, ID1:Rear_Right, ID4:Rear_Left
        combinations = []
        for r in range(1,len(list_of_lidar_ids_considered)+1):
            for combination in itertools.combinations(list_of_lidar_ids_considered, r):
                combinations.append(combination)
        #print(combinations)
        # Calculate the number of files each combination should be applied to
        num_samples_per_combination = int(len(self.ten_persent_list) / len(combinations))
        #print(num_samples_per_combination)
        #print(num_samples_per_combination)
        if sample_id in self.ten_persent_list:
            #self.ct += 1
            #print(self.ct)
            if self.num_samples_counter < num_samples_per_combination:
                self.num_samples_counter += 1
                lidarData = self.get_lidar(sample_id, combinations[self.lidar_id_combination_idx])
            else:
                self.num_samples_counter = 0
                self.lidar_id_combination_idx += 1
                if (self.lidar_id_combination_idx) < len(combinations):
                    lidarData = self.get_lidar(sample_id, combinations[self.lidar_id_combination_idx])
                else:
                    self.lidar_id_combination_idx = 0 
                    lidarData = self.get_lidar(sample_id, combinations[self.lidar_id_combination_idx])                
        ...
        img_file = img_file = self.get_image(sample_id)
    ...
    def get_image(self, idx):
        ...
        image_file_path = os.path.join(self.dataset_dir,'camera_lidar_semantic_bboxes',seq_name_,'camera','cam_front_center',file_name)
        # assert os.path.isfile(image_file_path)
        return image_file_path  # (H, W, C) -> (H, W, 3) OpenCV reads in BGR mode

    def get_lidar(self, idx, lidar_id_combination):
        list_of_lidar_ids_considered = lidar_id_combination
        seq_name_ = idx[0]
        seq_name = idx[1]
        uniq_id = idx[2]
        file_name = seq_name + '_lidar_' + 'frontcenter_' + uniq_id + '.npz'    
        lidar_file_path = os.path.join(self.dataset_dir,'camera_lidar_semantic_bboxes',seq_name_,'lidar','cam_front_center',file_name) 
        ...
        df = pd.concat([split_df, reflectance_df.reset_index(drop=True)], axis=1)
    ...
    def get_label(self, idx):
        ...
        label3D_file_path = os.path.join(self.dataset_dir,'camera_lidar_semantic_bboxes',seq_name_,'label3D','cam_front_center',file_name)        
        # assert os.path.isfile(label3D_file_path)
        return kitti_data_utils.read_label(label3D_file_path)
