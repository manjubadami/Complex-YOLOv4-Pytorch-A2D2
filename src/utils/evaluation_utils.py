...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for c in tqdm.tqdm(unique_classes, desc="Computing AP"):
        ...
        print('Total number of Ground truth for class {} is : {}'.format(c, n_gt))
        n_p = i.sum()  # Number of predicted objects
        print('Total number of Predected Box for class {} is : {}'.format(c, n_p))
        if n_p == 0 and n_gt == 0:
            continue
            ...
            rc.extend(recall_curve.tolist())
            r.append(recall_curve[-1])

            # Precision
            precision_curve = tpc / (tpc + fpc)
            pr.extend(precision_curve.tolist())
            p.append(precision_curve[-1])

            cls.extend([c] * len(recall_curve))
    ...
    df = pd.DataFrame({"Class":cls, "Precision":[float(f'{i:.4f}') for i in pr], "Recall":[float(f'{i:.4f}') for i in rc]})
    df.to_csv(os.path.join(os.getcwd(),'precision_recall.csv')) 
